...
module Handler =
    ...
    let withUrl<'TResult, 'TError> (url: string) (context: Context) = withUrlBuilder (fun _ -> url) context

    let fetch<'TError> (ctx: Context): HttpFuncResultPly<HttpResponseMessage, 'TError> =
