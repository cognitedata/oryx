...
module Retry =
    ...
    /// randomness.
    let rec retry<'T, 'TResult, 'TError> (shouldRetry: HandlerError<'TError> -> bool) (initialDelay: int<ms>) (maxRetries : int)  (next: HttpFunc<'T, 'TResult, 'TError>) (ctx: Context<'T>) : HttpFuncResult<'TResult, 'TError> = task {
