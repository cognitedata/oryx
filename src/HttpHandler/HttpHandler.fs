...
module HttpHandler =
    ...
    let runAsync<'TResult> (handler: IHttpHandler<'TResult>) = Core.runAsync<HttpContext, 'TResult> handler
    ...
    let withQuery (query: seq<struct (string * string)>) (source: IHttpHandler<'TSource>) : IHttpHandler<'TSource> =
