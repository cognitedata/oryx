...
let wikiDataItemsDecoders: Decoder<WikiSearchHits> =
    Decode.list wikiDataItemDecoder |> Decode.map WikiSearchHits
...
let query term =
    [ struct ("action", "opensearch"); struct ("search", term) ]

let request ctx term =
    ctx |> withQuery (query term) |> fetch |> json wikiDataItemsDecoders
...
let asyncMain _ =
    task {
        ...
        let ctx: HttpHandler<unit> =
            httpRequest |> GET |> withHttpClient client |> withUrl Url |> cache
